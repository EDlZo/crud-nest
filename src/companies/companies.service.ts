...
export class CompaniesService {
  ...
  async findAll(user?: AuthUser): Promise<Company[]> {
    try {
      ...
      // When ownership scoping is disabled (default) everyone sees all companies.
      // Admin/superadmin also always see everything regardless of enforcement flag.
      if (!enforceOwnership || isPrivileged) {
        ...
        console.log('CompaniesService.findAll - global view count:', companies.length);
        return companies;
      }
      
      // Ownership enforcement path for regular users
      if (!user) {
